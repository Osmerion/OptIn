...
object OsmerionFirOptInUsageBaseChecker {
    ...
    private fun FirRegularClass.loadExperimentalityForMarkerAnnotation(annotatedOwnerClassName: String? = null): Experimentality? {
        val experimental = getAnnotationByClassId(OsmerionOptInNames.REQUIRES_OPT_IN_CLASS_ID) ?: getAnnotationByClassId(OptInNames.REQUIRES_OPT_IN_CLASS_ID) ?: return null
        ...
        for ((annotationClassId, severity, message) in experimentalities) {
            if (!isExperimentalityAcceptableInContext(annotationClassId, context)) {
                val (diagnostic, verb) = when (severity) {
                    Experimentality.Severity.WARNING -> OsmerionFirErrors.OPT_IN_USAGE to "should"
                    Experimentality.Severity.ERROR -> OsmerionFirErrors.OPT_IN_USAGE_ERROR to "must"
                }
    ...
    fun reportNotAcceptedOverrideExperimentalities(
        ...
        for ((annotationClassId, severity, markerMessage, supertypeName) in experimentalities) {
            if (!symbol.fir.isExperimentalityAcceptable(annotationClassId) &&
                ...
                val (diagnostic, verb) = when (severity) {
                    Experimentality.Severity.WARNING -> OsmerionFirErrors.OPT_IN_OVERRIDE to "should"
                    Experimentality.Severity.ERROR -> OsmerionFirErrors.OPT_IN_OVERRIDE_ERROR to "must"
                }
    ...
    private fun isExperimentalityAcceptableInContext(
        ...
        if (fqNameAsString in languageVersionSettings.getFlag(AnalysisFlags.optIn)) return true
    ...
    private fun FirAnnotationContainer.isAnnotatedWithOptIn(annotationClassId: ClassId): Boolean {
        for (annotation in annotations) {
            ...
            val annotationClasses = when (coneType?.lookupTag?.classId) {
                OptInNames.OPT_IN_CLASS_ID -> annotation.findArgumentByName(OptInNames.USE_EXPERIMENTAL_ANNOTATION_CLASS)
                OsmerionOptInNames.OPT_IN_CLASS_ID -> annotation.findArgumentByName(Name.identifier("value"))
                else -> null
            } ?: continue
