...
/**
 * A {@link LocalChecker checker} that verifies that Kotlin's opt-in annotations are not used in Java code.
 *
 * @author  Leon Linhart
 */
public final class JavaAnnotationChecker implements LocalChecker {
        ...
        public Void visitAnnotation(AnnotationTree node, Void unused) {
            ...
            if (this.checkerContext.types().asElement(typeMirror) instanceof TypeElement typeElement) {
                switch (typeElement.getQualifiedName().toString()) {
                    case OptInProcessingContext.KOTLIN_OPT_IN_FQ_NAME,
                         OptInProcessingContext.KOTLIN_OPT_IN_REPEATED_FQ_NAME ->
                        this.checkerContext.reporter().error("com.osmerion.optin.OptIn should be used in Java code", node, this.compilationUnit);
                    case OptInProcessingContext.KOTLIN_REQUIRES_OPT_IN_FQ_NAME ->
                        this.checkerContext.reporter().error("com.osmerion.optin.RequiresOptIn should be used in Java code", node, this.compilationUnit);
                    case OptInProcessingContext.KOTLIN_SUBTYPING_REQUIRES_OPT_IN_FQ_NAME,
                         OptInProcessingContext.KOTLIN_SUBTYPING_REQUIRES_OPT_IN_REPEATED_FQ_NAME ->
                        this.checkerContext.reporter().error("com.osmerion.optin.SubtypingRequiresOptIn should be used in Java code", node, this.compilationUnit);
                }
            }
